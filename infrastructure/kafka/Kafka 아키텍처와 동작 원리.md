...
- [[dev/05-infrastructure/kafka/Kafka DLQ와 에러 처리|Kafka DLQ와 에러 처리]] — DLQ 패턴, Retry 전략, Spring Kafka 지원
- [[dev/05-infrastructure/kafka/Kafka Exactly-once Semantics|Kafka Exactly-once Semantics]] — 메시지 전달 보장, Idempotent Producer, Transaction
- [[dev/05-infrastructure/kafka/Kafka 메타데이터 관리, Zookeeper에서 KRaft로|Zookeeper에서 KRaft로]] — 클러스터 메타데이터 관리 방식 변화
- [[dev/04-architecture/마이크로서비스|마이크로서비스]] — Kafka가 주로 사용되는 MSA 환경
- [[dev/04-architecture/이벤트 드리븐 아키텍처|이벤트 드리븐 아키텍처]] — Kafka 기반 EDA 패턴
- [[dev/03-database/트랜잭션|트랜잭션]] — 분산 트랜잭션과 Kafka의 관계
- [[dev/06-computer-science/네트워크|네트워크]] — Zero-Copy, sendfile() 등 OS 레벨 최적화
- [[dev/09-system-design/building-blocks/Message Queue|Message Queue]] — 시스템 설계에서의 메시지 큐 활용
