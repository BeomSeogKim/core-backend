...
재시도 간격을 지수적으로 늘려가며 외부 서비스 부하를 줄이는 재시도 전략.
...
Exponential Backoff만으로는 여러 클라이언트가 동시에 실패했을 때 동시에 재시도하는 문제 발생.
...
**Jitter가 불필요한 경우**
- Single-flight 패턴으로 동일 요청이 이미 1개로 합쳐지는 경우
...
연속 실패가 임계치를 넘으면 요청 자체를 차단해 외부 서비스 회복 시간을 확보하는 패턴.
...
> **참고**: 위 구현은 Half-Open 상태 없이 Cooldown 만료 시 자동으로 Closed 복귀하는 단순화된 버전
